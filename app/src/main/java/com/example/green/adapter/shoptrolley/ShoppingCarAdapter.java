...
public class ShoppingCarAdapter extends BaseExpandableListAdapter {
    ...
    private List<ShoppingtrolleyListbean.ResultBean.CartListBean> data;
     ...
     * @param data 需要刷新的数据
     */
    public void setData(List<ShoppingtrolleyListbean.ResultBean.CartListBean> data) {
        this.data = data;
    ...
    public View getGroupView(final int groupPosition, final boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        final ShoppingtrolleyListbean.ResultBean.CartListBean datasBean = data.get(groupPosition);
        //店铺ID
        ...
        for (int i = 0; i < datasBean.getGoods().size(); i++) {
            ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean goodsBean = datasBean.getGoods().get(i);
        ...
        groupViewHolder.ll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                List<ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean> goods = datasBean.getGoods();
                for (int i = 0; i < goods.size(); i++) {
                    ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean goodsBean = goods.get(i);
        ...
        for (int i = 0; i < data.size(); i++) {
            List<ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean> goods = data.get(i).getGoods();
        ...
        btnOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                List<ShoppingtrolleyListbean.ResultBean.CartListBean> tempStores = new ArrayList<>();
                for (int i = 0; i < data.size(); i++) {
                    //店铺中是否有商品被选中
                    boolean hasGoodsSelect = false;
                    //创建临时的List，用于存储被选中的商品
                    List<ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean> tempGoods = new ArrayList<>();

                    ShoppingtrolleyListbean.ResultBean.CartListBean storesBean = data.get(i);
                    List<ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean> goods = storesBean.getGoods();
                    for (int y = 0; y < goods.size(); y++) {
                        ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean goodsBean = goods.get(y);
                        boolean isSelect = goodsBean.getIsSelect();
                        if (isSelect) {
                    ...
                    if (hasGoodsSelect) {
                        ShoppingtrolleyListbean.ResultBean.CartListBean storeBean = new ShoppingtrolleyListbean.ResultBean.CartListBean();
                ...
                if (tempStores != null && tempStores.size() > 0) {//如果有被选中的
                     ...
                     * 则跳转到确认订单页面，完成后续订单流程。
                     */
                    if (null != mOnSelectedGoodsListener) {
                        mOnSelectedGoodsListener.onSelectedGoods(tempStores);
                    }
                } else {
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        //店铺是否在购物车中被选中
        final boolean isSelect_shop = datasBean.getIsSelect_shop();
        final ShoppingtrolleyListbean.ResultBean.CartListBean.GoodsBean goodsBean = datasBean.getGoods().get(childPosition);
        //商品图片
        String goods_image = goodsBean.getGoods_image_url();
        //商品ID
        ...
        childViewHolder.ivEditAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mChangeCountListener != null) {
                    mChangeCountListener.onChangeCount(cart_id, goodsBean.getGoods_num());
                }
            }
        });
        //减号的点击事件
    ...
    public interface OnChangeCountListener {
        void onChangeCount(String cart_id, int goods_num);
    }
    ...
    // 传回所选商品回调
    public OnSelectedGoodsListener mOnSelectedGoodsListener;

    public interface OnSelectedGoodsListener {
        void onSelectedGoods(List<ShoppingtrolleyListbean.ResultBean.CartListBean> pCartListBeans);
    }

    public void setOnSelectedGoodsListener(OnSelectedGoodsListener pOnSelectedGoodsListener) {
        mOnSelectedGoodsListener = pOnSelectedGoodsListener;
    }
}
