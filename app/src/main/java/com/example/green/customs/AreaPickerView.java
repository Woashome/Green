...
import com.example.green.adapter.mine.AreaAdapter;
import com.example.green.adapter.mine.CityAdapter;
import com.example.green.adapter.mine.ProvinceAdapter;
...
public class AreaPickerView extends Dialog {
     ...
     * 市
     */
    private List<AddressBean.ResultBean.AreaListBeanXX.AreaListBeanX> cityBeans;
    /**
     * 区
     */
    private List<AddressBean.ResultBean.AreaListBeanXX.AreaListBeanX.AreaListBean> areaBeans;
    ...
    public AreaPickerView(@NonNull Context context, int themeResId, List<AddressBean.ResultBean.AreaListBeanXX> addressBeans) {
...
//                    addressBeans.get(oldProvinceSelected).setStatus(false);
                ...
                cityBeans.addAll(addressBeans.get(position).getArea_list());
...
//                cityBeans.get(position).setStatus(true);
                ...
                /*if (oldCitySelected != -1 && oldCitySelected != citySelected) {
                    addressBeans.get(oldProvinceSelected).getArea_list().get(oldCitySelected).setStatus(false);
                }*/
                if (position != oldCitySelected) {
                    if (oldAreaSelected != -1 && cityBeans.get(position).getArea_list() != null) {
//                        addressBeans.get(oldProvinceSelected).getChildren().get(oldCitySelected).getChildren().get(oldAreaSelected).setStatus(false);
                    }
                    oldAreaSelected = -1;
                }
                oldCitySelected = citySelected;
                if (cityBeans.get(position).getArea_list() != null) {
                    areaBeans.addAll(cityBeans.get(position).getArea_list());
                    cityAdapter.notifyDataSetChanged();
                    areaAdapter.notifyDataSetChanged();
...
//                areaBeans.get(position).setStatus(true);
    ...
    public void setSelect(int... value) {
        ...
        if (value.length == 3) {
            strings.add(addressBeans.get(value[0]).getArea_name());
            strings.add(addressBeans.get(value[0]).getArea_list().get(value[1]).getArea_name());
            strings.add(addressBeans.get(value[0]).getArea_list().get(value[1]).getArea_list().get(value[2]).getArea_name());
            tabLayout.setupWithViewPager(viewPager);
            viewPagerAdapter.notifyDataSetChanged();
            ...
            cityBeans.clear();
            cityBeans.addAll(addressBeans.get(value[0]).getArea_list());
            areaBeans.clear();
            areaBeans.addAll(addressBeans.get(value[0]).getArea_list().get(value[1]).getArea_list());
            provinceAdapter.notifyDataSetChanged();
            cityAdapter.notifyDataSetChanged();
            areaAdapter.notifyDataSetChanged();
